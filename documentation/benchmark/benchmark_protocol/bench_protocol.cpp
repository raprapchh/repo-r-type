...
namespace bench
{
    ...
    void runServer(int fd, int type)
    {
        ...
        if (type == SOCK_DGRAM) {
            ...
            for (std::size_t i = 0; i < kIterations; ++i) {
                if (::recvfrom(
                        fd, tmp.data(), tmp.size(), 0, reinterpret_cast<sockaddr*>(&client), &len) !=
        ...
        for (std::size_t i = 0; i < kIterations; ++i) {
            if (::recv(conn, tmp.data(), tmp.size(), MSG_WAITALL) !=
                static_cast<ssize_t>(tmp.size())) {
                fatal("tcp recv");
            }
    ...
    Stats runBenchmark(int type, SendFunc sendFunc, RecvFunc recvFunc)
    {
        sockaddr_in addr{};
        ...
        if (type == SOCK_STREAM) {
            stats.connectNs = std::chrono::duration_cast<std::chrono::nanoseconds>(
                                Clock::now() - connectStart)
                                .count();
        }
