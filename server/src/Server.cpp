...
void Server::start() {
    ...
    running_ = true;
    udp_server_->set_message_handler(
        [this](const std::string& ip, uint16_t port, const std::vector<uint8_t>& data) {
...
void Server::handle_client_message(const std::string& client_ip, uint16_t client_port,
    ...
    switch (static_cast<rtype::net::MessageType>(packet.header.message_type)) {
        ...
        case rtype::net::MessageType::Ping: {
            ...
            rtype::net::Packet pong_packet(
                static_cast<uint16_t>(rtype::net::MessageType::Pong),
                serializer.get_data());
...
void Server::handle_player_join(const std::string& client_ip, uint16_t client_port) {
    ...
    std::cout << "Player " << info.player_id << " joined from " << client_ip << ":"
              << client_port << std::endl;

    rtype::net::Serializer serializer;
    serializer.write(info.player_id);

    rtype::net::Packet response(static_cast<uint16_t>(rtype::net::MessageType::PlayerJoin),
                                serializer.get_data());
